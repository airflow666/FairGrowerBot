# Конфигурация бота
import os

from dotenv import load_dotenv

# Загружаем переменные окружения из .env
load_dotenv()

# Токен бота из переменных окружения
BOT_TOKEN = os.getenv("BOT_TOKEN")

# Имя бота для упоминаний (опционально)
BOT_USERNAME = os.getenv("BOT_USERNAME", "@FairDickGrowerBot")

# Путь к файлу базы данных (можно переопределить через переменную окружения)
DATABASE_PATH = os.getenv("DATABASE_PATH", "fairdick.db")

# Часовой пояс, по которому считаются "сутки" для grow и писюна дня
TIMEZONE = os.getenv("TIMEZONE", "Europe/Moscow")

# Ограничения для команды grow (включительно)
GROW_MIN = -10
GROW_MAX = 35

# Ограничения для бонуса "писюн дня"
DICK_OF_DAY_MIN = 1
DICK_OF_DAY_MAX = 30

# Дуэль: доступные ставки и время жизни вызова (в секундах)
DUEL_STAKES = [1, 2, 3, 4, 5]
DUEL_TIMEOUT = 120

# Казино: доступные ставки (в монетах) и множитель выигрыша. Ставки крупные,
# показываются только те, что игрок может себе позволить (см. cmd_casino).
CASINO_STAKES = [10, 50, 250, 1000, 5000]
CASINO_WIN_MULTIPLIER = 2

# --- RPG: персонаж, опыт, классы ---------------------------------------------

# Опыт за действия
EXP_PER_GROW = 10
EXP_PER_DUEL_WIN = 15
EXP_PER_DUEL_LOSS = 5

# Кривая уровней: опыт для перехода с уровня L на L+1 = LEVEL_STEP * L
LEVEL_STEP = 100

# Характеристики: база в каждой + очки за уровень, распределяемые по классу
BASE_STAT = 5
POINTS_PER_LEVEL = 3
# Плюс свободные очки за уровень — игрок вкладывает их сам (кнопки в профиле)
FREE_POINTS_PER_LEVEL = 2

# Пассивки классов (не имба, но ощутимо; все работают в подземельях)
PASSIVE_FIRST_STRIKE_BONUS = 0.5   # giga: первый удар по мобу +50%
PASSIVE_DAMAGE_REDUCTION = 0.15    # tank: урон мобов и ловушек -15%
PASSIVE_LOOT_CHANCE_BONUS = 0.10   # lucky: +10% к шансу лута (мобы и тайники)
PASSIVE_CRIT_LIFESTEAL = 3         # crit: крит в бою лечит на N HP
PASSIVE_XP_BONUS = 0.10            # avg: +10% опыта отовсюду
# speed: побег от моба всегда без урона (см. dungeon.flee)

# Дуэль: шанс победы = 0.5 + PER_POWER * (сила_атакующего - сила_защитника),
# ограниченный коридором [MIN, MAX], чтобы новичков нельзя было фармить
DUEL_CHANCE_MIN = 0.35
DUEL_CHANCE_MAX = 0.65
DUEL_CHANCE_PER_POWER = 0.02
# Длина тоже решает: +1% к шансу за каждые DUEL_CM_PER_PERCENT см разницы
DUEL_CM_PER_PERCENT = 50

# Титулы по длине (чатовой): (порог_см, эмодзи, название). Проверяются сверху вниз.
TITLES = [
    (2500, "🚀", "Орбитальный Хуй Возмездия"),
    (1200, "🚂", "Царь-Елдак"),
    (600, "🐍", "Анаконда Подъезда"),
    (300, "🍆", "Уважаемый Шланг"),
    (100, "🌭", "Хер с Горы"),
    (0, "🥒", "Корнишон"),
    (-10**9, "💸", "Микропенис с Ипотекой"),
]

# Классы: code -> параметры. weights — распределение очков по 5 статам.
# Коды giga/lucky/tank сохранены (у старых игроков класс не слетает).
CLASSES = {
    "giga": {
        "emoji": "💪", "name": "Гигачлен", "perk": "Ебёт в дуэлях и по боссу",
        "weights": {"strength": 0.7, "vitality": 0.1, "luck": 0.05,
                    "crit": 0.1, "speed": 0.05},
        "passive": ("🥊", "С Порога в Ебало", "первый удар по мобу +50% урона"),
    },
    "tank": {
        "emoji": "🛡️", "name": "Мясной Штурмовик", "perk": "Танкует подземелья лицом",
        "weights": {"strength": 0.1, "vitality": 0.7, "luck": 0.05,
                    "crit": 0.05, "speed": 0.1},
        "passive": ("🧱", "Дублёная Шкура", "урон мобов и ловушек -15%"),
    },
    "lucky": {
        "emoji": "🍀", "name": "Лудоман", "perk": "Лут липнет как к мёду",
        "weights": {"strength": 0.05, "vitality": 0.1, "luck": 0.7,
                    "crit": 0.05, "speed": 0.1},
        "passive": ("🧲", "Мародёр", "+10% к шансу лута с мобов и тайников"),
    },
    "crit": {
        "emoji": "💥", "name": "Хуеглот-Берсерк", "perk": "Рампага по боссу",
        "weights": {"strength": 0.15, "vitality": 0.1, "luck": 0.05,
                    "crit": 0.6, "speed": 0.1},
        "passive": ("🩸", "Жажда Крови", "крит в бою лечит на 3 HP"),
    },
    "speed": {
        "emoji": "⚡", "name": "Скорострел", "perk": "Кончает экспедиции первым",
        "weights": {"strength": 0.1, "vitality": 0.1, "luck": 0.1,
                    "crit": 0.05, "speed": 0.65},
        "passive": ("💨", "Скользкий Тип", "побег от моба всегда без урона"),
    },
    "avg": {
        "emoji": "🗿", "name": "Среднестатистический Хуй", "perk": "Всё поровну, зато стабильно",
        "weights": {"strength": 0.2, "vitality": 0.2, "luck": 0.2,
                    "crit": 0.2, "speed": 0.2},
        "passive": ("📈", "Душный Гринд", "+10% опыта отовсюду"),
    },
}

# Характеристики: code -> (эмодзи, название, что делает — для пояснений)
STATS = {
    "strength": ("💪", "Сила", "урон в дуэлях и по боссу"),
    "vitality": ("❤️", "Живучесть", "запас HP в подземелье"),
    "luck": ("🍀", "Удача", "шанс редкого лута"),
    "crit": ("💥", "Крит", "шанс удвоить урон по боссу"),
    "speed": ("⚡", "Скорость", "ускоряет экспедиции"),
}

# --- RPG: редкости, предметы, зоны -------------------------------------------

# Редкости: code -> (эмодзи, название, вес в лут-таблице, множитель бонуса).
# 9 тиров: ⚪🟢🔵🟣🟠🔴 + звёздные ⭐ relic / ⭐⭐ cosmic / ⭐⭐⭐ godlike. Веса
# верхних тиров срезаны, чтобы эпик+ не фармился, а звёзды были редчайшими «в
# диких» (но крафтятся). Порядок важен (от common к godlike) — для лут-роллов.
RARITIES = {
    "common":    ("⚪", "Сперма",              50.0, 2),
    "uncommon":  ("🟢", "Прям Хуйня",          28.0, 4),
    "rare":      ("🔵", "Залупа",              14.0, 7),
    "epic":      ("🟣", "Говно Мамонта",        2.2, 11),
    "legendary": ("🟠", "Ебовина",              0.5, 16),
    "mythic":    ("🔴", "Пиздатость",           0.12, 24),
    "relic":     ("⭐", "Охуевшая Легенда",     0.02, 36),
    "cosmic":    ("⭐⭐", "Космический Пиздец",   0.004, 52),
    "godlike":   ("⭐⭐⭐", "Абсолютная Елда Мироздания", 0.0008, 75),
}

# Редкость: factor = 1 + luck_boost + zone_bonus, где luck_boost = √-затухающий
# вклад Удачи со своим капом (LUCK_BOOST_CAP), а zone_bonus — бонус зоны/сундука.
# Итоговый factor ограничен RARITY_FACTOR_CAP. Раньше кап накрывал СУММУ, и на
# дорогих сундуках (bonus ≥ 0.5) Удача упиралась в потолок и не влияла вообще —
# теперь у вклада Удачи отдельный кап, поэтому она работает и поверх бонуса.
LUCK_RARITY_FACTOR = 0.04
LUCK_BOOST_CAP = 0.4     # максимальный вклад Удачи в factor (≈luck 100)
RARITY_FACTOR_CAP = 2.2  # общий потолок (bonus ≤0.8 + Удача помещаются под ним)

# Модель статов предмета: (мин, макс) число статов и бюджет очков по редкости.
# Статы выбираются СЛУЧАЙНО из всех пяти, бюджет случайно делится между ними —
# два одинаковых по шаблону предмета почти никогда не совпадают по статам.
ITEM_STAT_COUNT = {
    "common": (1, 1), "uncommon": (1, 2), "rare": (2, 2),
    "epic": (2, 3), "legendary": (3, 3), "mythic": (3, 4), "relic": (4, 5),
    "cosmic": (5, 5), "godlike": (5, 5),
}
ITEM_STAT_BUDGET = {
    "common": 2, "uncommon": 4, "rare": 7,
    "epic": 11, "legendary": 16, "mythic": 24, "relic": 36,
    "cosmic": 52, "godlike": 75,
}

# Слоты экипировки: code -> (эмодзи, название, тематический стат для фолбэка
# старых предметов). Статы новых предметов роллятся случайно, слот — косметика.
SLOTS = {
    "weapon":   ("⚔️", "Оружие", "strength"),
    "helmet":   ("🪖", "Шлем", "crit"),
    "armor":    ("🎽", "Броня", "vitality"),
    "boots":    ("👢", "Сапоги", "speed"),
    "artifact": ("💍", "Артефакт", "luck"),
}

# Названия предметов: slot -> rarity -> [названия]. Из них строится каталог.
_ITEM_NAMES = {
    "weapon": {
        "common": ["Ржавый Хуерез", "Деревянный Дрын", "Погнутая Вилка"],
        "uncommon": ["Дилдо-катана", "Резиновая Дубина", "Заточка Зэка"],
        "rare": ["Хуерубило +2", "Кувалда Правосудия", "Тесак Мясника"],
        "epic": ["Клинок Вечного Стояка", "Молот Гачимучи", "Коса Импотента"],
        "legendary": ["Двуручный Елдак", "Топор Батиного Ремня"],
        "mythic": ["Пенетратор 3000", "Меч-Хуекладенец"],
        "relic": ["Экскалибур из Штанов", "Аннигилятор Целок"],
        "godlike": ["Абсолютный Хуерез Мироздания"],
        "cosmic": ["Хуй Господень", "Меч Большого Взрыва"],
    },
    "helmet": {
        "common": ["Шлем Налысо", "Дырявая Панама", "Каска Куколда"],
        "uncommon": ["Ведро на Башку", "Тарелка НЛО", "Кепка Гопника"],
        "rare": ["Шлем Гвардейца-Дрочера", "Корона из Фольги"],
        "epic": ["Венец Ярости", "Башка Терминатора"],
        "legendary": ["Шлем Берсерка-Задрота"],
        "mythic": ["Венец Палача Целок"],
        "relic": ["Корона Хуёвого Короля"],
        "godlike": ["Корона Творца Вселенных"],
        "cosmic": ["Нимб Гачимучи", "Шлем Галактического Императора"],
    },
    "armor": {
        "common": ["Обоссанная Майка", "Дырявые Труселя", "Тряпьё Бомжа"],
        "uncommon": ["Кольчуга из Скрепок", "Косуха Гопника"],
        "rare": ["Латы Стражника-Алкаша", "Броник ЧОПа"],
        "epic": ["Броня Титана", "Экзоскелет Задрота"],
        "legendary": ["Панцирь Дракона-Импотента"],
        "mythic": ["Доспех Бессмертного Дрочера"],
        "relic": ["Эгида Эрекции"],
        "godlike": ["Эгида Большого Взрыва"],
        "cosmic": ["Броня Звёздного Дрочера", "Панцирь Чёрной Дыры"],
    },
    "boots": {
        "common": ["Стоптанные Лапти", "Дырявые Кеды", "Тапки Бати"],
        "uncommon": ["Кроксы Смерти", "Берцы Военкома"],
        "rare": ["Сапоги-Скороходы", "Найки Гопника"],
        "epic": ["Сапоги Побегушек от Алиментов", "Кроссы Ветра"],
        "legendary": ["Крылатые Сандалии Гермеса"],
        "mythic": ["Поступь Ебущей Бури"],
        "relic": ["Сапоги-Телепорт в ЛП"],
        "godlike": ["Поступь по Головам Богов"],
        "cosmic": ["Сапоги Скорости Света", "Кроссы Варп-Двигателя"],
    },
    "artifact": {
        "common": ["Кроличья Лапка", "Ржавый Пятак", "Амулет Вялого Стояка"],
        "uncommon": ["Кольцо Лудомана", "Оберег от Военкома"],
        "rare": ["Око Фортуны", "Медаль «За Дрочку»"],
        "epic": ["Талисман Стояка", "Печать Гачимучи"],
        "legendary": ["Звезда Лудомана"],
        "mythic": ["Слеза Импотента"],
        "relic": ["Кольцо Анального Всевластия"],
        "godlike": ["Кольцо Всех Колец"],
        "cosmic": ["Сингулярность в Кармане", "Ядро Нейтронной Звезды"],
    },
}


def _build_item_templates():
    """Каталог предметов: code -> {name, slot, rarity}. Коды стабильны."""
    catalog = {}
    for slot, by_rarity in _ITEM_NAMES.items():
        for rarity, names in by_rarity.items():
            for i, name in enumerate(names):
                catalog[f"{slot}_{rarity}_{i}"] = {
                    "name": name, "slot": slot, "rarity": rarity,
                }
    return catalog


ITEM_TEMPLATES = _build_item_templates()

# Зоны экспедиций: code -> параметры. duration — в секундах.
ZONES = {
    "meadow": {
        "emoji": "🌾", "name": "Луг Вялого Стояка", "min_level": 1,
        "duration": 3600, "exp": 20, "coins": (5, 15), "luck_bonus": 0.0,
    },
    "forest": {
        "emoji": "🌲", "name": "Лес Импотенции", "min_level": 3,
        "duration": 4 * 3600, "exp": 60, "coins": (20, 50), "luck_bonus": 0.1,
    },
    "caves": {
        "emoji": "🕳️", "name": "Пещера Простаты", "min_level": 6,
        "duration": 8 * 3600, "exp": 150, "coins": (60, 120), "luck_bonus": 0.25,
    },
    # Старшие зоны для прокачанных: дольше, но жирнее опыт/монеты, а loot_floor
    # гарантирует минимальную редкость добычи (как у подземелий).
    "swamp": {
        "emoji": "🐸", "name": "Болото Хронической Импотенции", "min_level": 12,
        "duration": 12 * 3600, "exp": 350, "coins": (150, 300), "luck_bonus": 0.35,
        "loot_floor": "uncommon",
    },
    "peaks": {
        "emoji": "🏔️", "name": "Пик Дрочащего Йети", "min_level": 18,
        "duration": 18 * 3600, "exp": 700, "coins": (300, 600), "luck_bonus": 0.5,
        "loot_floor": "rare",
    },
    "void": {
        "emoji": "🌌", "name": "Бездна Вечного Целибата", "min_level": 25,
        "duration": 24 * 3600, "exp": 1400, "coins": (600, 1200), "luck_bonus": 0.6,
        "loot_floor": "epic",
    },
}

# Порядок редкостей от обычного к реликтовому (для лут-роллов)
RARITY_ORDER = list(RARITIES.keys())

# Максимум предметов в инвентаре для показа (чтобы не раздувать сообщение)
INVENTORY_DISPLAY_LIMIT = 20

# --- RPG: боссы --------------------------------------------------------------

# Боссы: из списка выбирается случайный при спавне
BOSS_TEMPLATES = [
    {"emoji": "👹", "name": "Микропенис с Ипотекой", "hp": 300},
    {"emoji": "🐉", "name": "Хуеглот с Рампагой", "hp": 600},
    {"emoji": "💀", "name": "Древний Дилдак Анальной Секты", "hp": 900},
]

BOSS_HIT_COOLDOWN = 3600      # секунд между ударами одного игрока
BOSS_BASE_DAMAGE = 5          # к урону прибавляется Сила игрока
BOSS_DAMAGE_PER_CM = 100      # +1 урона по боссу за каждые N см длины (чатовой)
BOSS_EXP_REWARD = 80          # опыт каждому участнику при победе
BOSS_COIN_POOL = 400          # монеты, делятся между бойцами по вкладу урона

# Дроп с босса — ОТДЕЛЬНАЯ таблица: гарантированный «пол» редкости по доле
# урона (проверяется сверху вниз), дальше ролл от пола и выше. Босс всегда
# даёт лучше подземелий и сундуков.
BOSS_LOOT_FLOORS = [
    (0.40, "legendary"),
    (0.20, "epic"),
    (0.08, "rare"),
    (0.0, "uncommon"),
]
BOSS_TOP_TIER_UP_CHANCE = 0.35  # шанс топ-дамагеру поднять пол ещё на +1 тир

# --- RPG: подземелья (комнаты: мобы/сокровища/ловушки/привалы) ----------------

DUNGEON_BASE_HP = 50              # базовое HP + Живучесть * множитель
DUNGEON_HP_PER_VITALITY = 5
DUNGEON_EXP_PER_DEPTH = 8         # опыт за пройденную глубину (при выходе живым)

# Типы комнат и их вероятности (сумма = 1). Мобы — главный источник наград.
DUNGEON_ROOM_WEIGHTS = [
    ("mob", 0.40),
    ("treasure", 0.30),
    ("trap", 0.20),
    ("rest", 0.10),
]

# Бой с мобом (в один клик): Сила = урон, Крит удваивает (кап как у босса),
# Скорость = шанс уклониться от каждого удара моба, Живучесть = запас HP.
DUNGEON_DODGE_PER_POINT = 0.01
DUNGEON_DODGE_CAP = 0.5
# Побег: получаешь до FLEE_DAMAGE от силы моба (уклон работает), но не умираешь.
DUNGEON_FLEE_DAMAGE = 0.5

# Подземелья: code -> параметры. Гейтинг по уровню. Награды РАНДОМНЫЕ
# (диапазоны) и растут с тиром и глубиной. loot_floor — минимальная редкость
# лута отсюда; loot_bonus — сдвиг ролла вверх (фиксированный).
# mob_*: hp/power — диапазон + прирост за глубину; bounty — монеты за победу.
DUNGEONS = {
    "rats": {
        "emoji": "🕸️", "name": "Подвал Микропениса", "min_level": 1,
        "entry_cost": 20,
        "mobs": [("🐀", "Крыса-Качок"), ("🪳", "Таракан-Гопник"),
                 ("🧦", "Носок Судьбы")],
        "mob_hp": (15, 25), "mob_hp_per_depth": 3,
        "mob_power": (4, 7), "mob_power_per_depth": 1,
        "mob_bounty": (15, 30), "mob_bounty_per_depth": 4, "mob_item_chance": 0.35,
        "coins_room": (8, 20), "coins_per_depth": 4, "item_chance": 0.20,
        "trap_damage": 5, "trap_per_depth": 3, "rest_heal": (5, 15),
        "loot_floor": "common", "loot_bonus": 0.0,
    },
    "fort": {
        "emoji": "🏰", "name": "Ипотечное Гетто", "min_level": 5,
        "entry_cost": 90,
        "mobs": [("🧟", "Зомби-Должник"), ("👮", "Коллектор"),
                 ("🤡", "Клоун-Риелтор")],
        "mob_hp": (35, 55), "mob_hp_per_depth": 5,
        "mob_power": (8, 14), "mob_power_per_depth": 2,
        "mob_bounty": (40, 75), "mob_bounty_per_depth": 6, "mob_item_chance": 0.40,
        "coins_room": (20, 50), "coins_per_depth": 6, "item_chance": 0.25,
        "trap_damage": 10, "trap_per_depth": 5, "rest_heal": (10, 25),
        "loot_floor": "uncommon", "loot_bonus": 0.10,
    },
    "canals": {
        "emoji": "🚽", "name": "Канализация Разбитых Надежд", "min_level": 10,
        "entry_cost": 250,
        "mobs": [("🐊", "Крокодил из Унитаза"), ("🦠", "Хламидия-Переросток"),
                 ("🧌", "Тролль Канализации")],
        "mob_hp": (60, 90), "mob_hp_per_depth": 7,
        "mob_power": (14, 22), "mob_power_per_depth": 3,
        "mob_bounty": (80, 160), "mob_bounty_per_depth": 11, "mob_item_chance": 0.45,
        "coins_room": (45, 100), "coins_per_depth": 11, "item_chance": 0.28,
        "trap_damage": 18, "trap_per_depth": 8, "rest_heal": (15, 35),
        "loot_floor": "uncommon", "loot_bonus": 0.20,
    },
    "dragon": {
        "emoji": "🌋", "name": "Жерло Хуеглота", "min_level": 16,
        "entry_cost": 600,
        "mobs": [("🦎", "Ящер-Дрочер"), ("🔥", "Огненный Хуезавр"),
                 ("🐲", "Драконыш-Пироман")],
        "mob_hp": (90, 140), "mob_hp_per_depth": 9,
        "mob_power": (22, 34), "mob_power_per_depth": 4,
        "mob_bounty": (160, 320), "mob_bounty_per_depth": 20, "mob_item_chance": 0.50,
        "coins_room": (90, 200), "coins_per_depth": 20, "item_chance": 0.30,
        "trap_damage": 30, "trap_per_depth": 12, "rest_heal": (25, 50),
        "loot_floor": "rare", "loot_bonus": 0.30,
    },
    "hell": {
        "emoji": "😈", "name": "Ад Алиментщика", "min_level": 22,
        "entry_cost": 1500,
        "mobs": [("👿", "Бес-Алиментщик"), ("💼", "Демон Просроченных Зарплат"),
                 ("☠️", "Аудитор Ада")],
        "mob_hp": (140, 220), "mob_hp_per_depth": 12,
        "mob_power": (34, 50), "mob_power_per_depth": 6,
        "mob_bounty": (350, 700), "mob_bounty_per_depth": 40, "mob_item_chance": 0.55,
        "coins_room": (200, 450), "coins_per_depth": 40, "item_chance": 0.32,
        "trap_damage": 50, "trap_per_depth": 18, "rest_heal": (40, 80),
        "loot_floor": "rare", "loot_bonus": 0.45,
    },
}

# --- RPG: экономика (магазин, пассивный доход) -------------------------------

# Сундуки в магазине: code -> параметры. bonus смещает редкость лута к редкой,
# floor (если задан) — гарантированная минимальная редкость дропа. Удача игрока
# слегка повышает шанс редкого (тот же √-затухающий фактор, что и в экспедициях).
SHOP_CHESTS = {
    "wooden":   {"emoji": "📦", "name": "Деревянный сундук", "price": 50,   "bonus": 0.0},
    "silver":   {"emoji": "🎁", "name": "Серебряный сундук", "price": 150,  "bonus": 0.3},
    "golden":   {"emoji": "💎", "name": "Золотой сундук",    "price": 400,  "bonus": 0.8},
    "platinum": {"emoji": "🧰", "name": "Платиновый сундук",  "price": 1200,
                 "bonus": 0.6, "floor": "rare"},
    "royal":    {"emoji": "👑", "name": "Королевский сундук", "price": 4000,
                 "bonus": 0.8, "floor": "epic"},
}

# Стоимость смены класса (сброс/выбор нового)
CLASS_CHANGE_COST = 200

# Уровни фермы (пассивный доход). upgrade_cost — цена перехода на этот уровень.
PROPERTIES = [
    {"emoji": "🌱", "name": "Грядка", "upgrade_cost": 100, "rate_per_hour": 5},
    {"emoji": "🪴", "name": "Теплица", "upgrade_cost": 300, "rate_per_hour": 15},
    {"emoji": "🏡", "name": "Плантация", "upgrade_cost": 800, "rate_per_hour": 40},
    {"emoji": "🚜", "name": "Агрохолдинг «Елдак-Агро»", "upgrade_cost": 2000,
     "rate_per_hour": 90},
    {"emoji": "🏭", "name": "Завод Смазки", "upgrade_cost": 5000,
     "rate_per_hour": 200},
    {"emoji": "🛢️", "name": "Нефтевышка «Эль Дилдорадо»", "upgrade_cost": 12000,
     "rate_per_hour": 450},
]

# Накопление дохода капается на столько часов (стимул заходить и собирать)
PROPERTY_CAP_HOURS = 24

# Цена продажи предмета по редкости (монеты)
SELL_PRICES = {
    "common": 5, "uncommon": 12, "rare": 30, "epic": 80,
    "legendary": 200, "mythic": 500, "relic": 1200,
    "cosmic": 3000, "godlike": 8000,
}

# Обмен сантиметров на монеты (односторонний, чтобы держать длину умеренной)
CM_PER_COIN = 10
CONVERT_OPTIONS = [50, 100, 500]  # сколько см можно обменять за раз

# Крафт: сколько предметов одного тира нужно для 1 предмета тира выше
CRAFT_ITEMS_REQUIRED = 5

# Крит по боссу: шанс удвоить урон = crit * BOSS_CRIT_PER_POINT, с капом
BOSS_CRIT_PER_POINT = 0.01
BOSS_CRIT_CAP = 0.5

# Скорость сокращает время экспедиции: speed * SPEED_PER_POINT, с капом
EXPEDITION_SPEED_PER_POINT = 0.01
EXPEDITION_SPEED_CAP = 0.5


# Достижения: code -> (эмодзи, название)
ACHIEVEMENTS = {
    "first_grow": ("🌱", "Первый Раз (не считается)"),
    "size_50": ("📏", "Полметра Счастья"),
    "size_100": ("🍆", "Метровый Монстр"),
    "jackpot": ("💥", "Бафнули в Патче (+35 за раз)"),
    "unlucky": ("💀", "Понерфили (-10 за раз)"),
    "streak_7": ("🔥", "Дрочу Неделю Подряд"),
    "duel_win_1": ("⚔️", "Первая Кровь"),
    "duel_win_10": ("👑", "Тащер (10 побед)"),
    "dick_of_day": ("🎉", "Писюн Дня"),
    # RPG-ачивки
    "boss_slayer": ("🐉", "Затащил Босса"),
    "deep_diver": ("🕳️", "Погружение на 10 (глубина ≥10)"),
    "star_owner": ("⭐", "Звёздный Барыга (звёздная шмотка)"),
    "rich": ("🪙", "Магнат (10 000 монет)"),
}

# Глубина подземелья для ачивки «Погружение»
ACH_DEEP_DIVE_DEPTH = 10
# Порог баланса для ачивки «Магнат»
ACH_RICH_COINS = 10000
