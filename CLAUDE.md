...
- **9 тиров редкости** (`RARITIES`): ⚪🟢🔵🟣🟠🔴 + звёздные ⭐ relic / ⭐⭐ cosmic /
  ⭐⭐⭐ godlike (коды в БД стабильны — переименование эмодзи миграции не требует).
...
- Сундуки (`SHOP_CHESTS`): 5 шт., поле `floor` (если задано) — гарантированная
  минимальная редкость через `loot.generate_floored` (платиновый ≥🔵, королевский
  ≥🟣); Удача игрока слегка двигает ролл вверх (тот же √-фактор). Дорогие дают
  жирнее, но золото с подземелий подрезано (`mob_bounty/coins_room`), чтобы вход
  в высокий тир окупался умеренно, а не печатал монеты.
